...
For this practice problem, use the following criteria to determine
...
valid email rules:
must contain one @
local part contains one or more letters A-Z/a-z and/or digits 0-9 only, no other chars
domain part contains two or more components separated by a dot (.) and must contain
one or more letters A-Z/a-z only, no other chars
*/

// function isValidEmail(email) {
//   let matches = email.match(/([a-z0-9]+@[a-z]+[.][a-z]{2,}([.][a-z]{2,})*)/i);
...
// function isValidEmail(email) {
//   let matches = email.match(/(^[a-z0-9]+@[a-z]+(\.[a-z]{2,})+)/i);
//   return matches ? true : false;
// }

function isValidEmail(email) {
  return /^[a-z0-9]+@[a-z]+(.[a-z]{2,})+/i.test(email);
}
